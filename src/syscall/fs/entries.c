...
ssize_t syscall_routine_getdents(size_t fd, void *buf, size_t count, proc_t *proc, pml4_t *current_pml4) {
    ...
    if (!buf_tmp) {
        printf("syscall_routine_getdents: failed to allocate temporary buffer for read of %p bytes for process %s (pid %p)\n",
    ...
    if (res > 0) {
        ...
        if (check < 0) {
            printf("syscall_routine_getdents: failed to copy data to user buffer %p for process %s (pid %p), errno %d\n", buf,
